			...
			if($num_rows == 0){
				...
				while ($sqlfetch = $result->fetch_assoc()){ 
					...
					print_r($sqlfetch);
					 ...
					 $sqlUpdate='UPDATE eol_prod.user_profile SET can_migrate=0 WHERE mobile_number='.$sqlfetch['mobile_number'];
...
WHERE
	...
	public function nameMatching($l1,$l2){
...
HAVING COUNT(up.mobile_number) > 1 and COUNT(up.mobile_number) <=2 limit $l1,$l2 ;";
...
if($argv[1]=='loginid'){	
	$userProfile->nameMatching($argv[2],$argv[3]);
}
