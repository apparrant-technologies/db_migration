	...
	public function mobileVal($role_id){
		...
		FROM
			...
			where sm.school_id not in (1,2,12,39,44,51,59,60,61,66,70,71,171,268,287,754,1116,1269,1441)
			   and ((up.mobile_number ='' OR up.mobile_number not regexp '^(9|8|7)' OR  up.mobile_number not regexp '[0-9]{10}' )   OR (`up`.`mobile_number` = '9999999999')
            OR (`up`.`mobile_number` ='8888888888')
            OR (`up`.`mobile_number`= '7777777777')) ; 
...
HAVING COUNT(up.mobile_number) > 1 and COUNT(up.mobile_number) <=2 limit 0,20000 ;";
